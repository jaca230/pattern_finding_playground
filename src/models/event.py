...
class Event:
    def __init__(self, event_id: int):
        ...
        self.all_vertices: Set[Vertex] = set()
        self.all_hits: list[Hit] = []
    ...
    def __repr__(self) -> str:
        ...
        vertex_ids = [getattr(v, 'vertex_id', repr(v)) for v in list(self.all_vertices)[:3]]
        hits_summary = f"{len(self.all_hits)} hits" if self.all_hits else "no hits"
        ...
        vertices_summary = (f"{len(self.all_vertices)} vertices (sample IDs: {vertex_ids})"
                            if self.all_vertices else "no vertices")
        ...
        return (f"Event(id={self.event_id}, {patterns_summary}, "
                f"{tracklets_summary}, {vertices_summary}, {hits_summary}, extra_info: {extra_info_summary}, "
