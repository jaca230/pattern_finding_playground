...
class EventInputStage(Stage):
    ...
    def build_handler(self):
        def handler(storage, input_context):
            ...
            storage.setdefault("extra_info", {})["geo"] = input_context.reco_data.geo
            ...
            storage["raw_hits"] = self._build_raw_hits(storage["reco_entry"], storage["geo"])
            if not input_context.use_truth:
                storage["truth_entry"] = None
            elif input_context.truth_data is not None:
                storage["truth_entry"] = input_context.truth_data.load_entry(input_context.entry_index)
            else:
                storage["truth_entry"] = storage["reco_entry"].get("truth_entry")
        return handler

    def _build_raw_hits(self, reco_entry, geo):
        hits = []
        for hit in reco_entry["hits"]:
            vid = hit.GetVID()
...
class InputContext:
    __slots__ = ("reco_data", "entry_index", "truth_data", "use_truth")

    def __init__(self, reco_data, entry_index, truth_data=None, use_truth=True):
